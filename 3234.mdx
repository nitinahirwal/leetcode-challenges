---
title: LeetCode Challenge Day 61 — 3234. Count the Number of Substrings With Dominant Ones
summary: Day 61 of my LeetCode challenge! Today's problem is "3234. Count the Number of Substrings With Dominant Ones" — an optimization problem that uses smart enumeration and mathematical counting to avoid brute force.
...
Today's problem is **3234. Count the Number of Substrings With Dominant Ones** — a clever counting problem where we need to find substrings where the number of '1's is at least the square of the number of '0's.
...
In other words, for a substring with `zeros` zeros and `ones` ones, we need: `ones ≥ zeros²`
...
1. **Substrings with no zeros**: These are all-ones substrings, which always satisfy the condition (since 1s ≥ 0²).
2. **Substrings with k zeros**: We need at least k² ones for the condition to hold.

For case 2, we can't enumerate all possible substrings (too slow). Instead, we fix the exact k zeros that appear in the substring and count how many ways we can extend left and right while maintaining ones ≥ k². Since k² grows quickly, we only need to check k up to √n (larger k would require more than n characters).

---

## 🔑 Approach
1. **Count all-ones substrings**: Find each maximal run of consecutive '1's. A run of length L contributes L×(L+1)/2 substrings.

2. **For each valid k (1 to √n)**:
   ...
   - For each possible selection of k consecutive zeros (by their positions):
     ...
     - Determine how many additional ones we need from left/right extensions: `need = k² - base`
     - Count valid combinations of left extensions (L choices) and right extensions (R choices)
     - If `left + right ≥ need`, the substring is valid
     - Use a helper function to efficiently count pairs (x, y) where x + y ≥ need

3. **Helper function `countPairsLE(L, R, t)`**: Counts pairs where x ∈ [0, L-1], y ∈ [0, R-1], and x + y ≤ t. We use this to find bad pairs (x + y < need), then subtract from total pairs.
...
The substring "11" at the end contributes: 2×(2+1)/2 = 3 substrings
- "1" (index 3)
- "1" (index 4)  
- "11" (index 3-4)
...
For each single '0':
- First '0' at index 0: Can extend right to include ones, giving us valid substrings
- Second '0' at index 1: Similarly can form valid substrings
- Third '0' at index 2: Can include the '11' on the right

### **Final Result**
After counting all valid combinations, we get the total count of substrings where ones ≥ zeros².
...
- **Time Complexity:**  
  ...
  We iterate through at most √n values of k, and for each k, we iterate through O(n) possible windows of k zeros. Each window is processed in O(1) time with the helper function.
...
## 🎯 Reflection
This problem demonstrates the power of **mathematical optimization** over brute force. By recognizing that k can only go up to √n and using smart pair counting, we transform an intractable O(n³) problem into an efficient O(n√n) solution.

Key insights:
- All-ones substrings are always valid (ones ≥ 0²)
- For k zeros, we need k² ones, limiting k to √n
...
See you tomorrow for **Day 62** — keep the momentum alive! 💪
